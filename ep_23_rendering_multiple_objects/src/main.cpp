...
GLFWwindow* initWindow()
{
	...
	GLFWwindow* window = glfwCreateWindow( 1024, 768, "OpenGL Tutorial - Rendering multiple objects", nullptr, nullptr );
	if ( window == nullptr )
	{
		return nullptr;
	}
...

	{
				...
				&& ( glfwGetKey( window, GLFW_KEY_ESCAPE ) != GLFW_PRESS ) )
		{
...

			{
				...
				glm::mat4 mvp = proj * view * model;
				shader.Bind();
				shader.SetUniform4f( "u_Color", 1.0f - red, 0.3f, 0.8f, 1.0f );
				shader.SetUniformMat4f( "u_MVP", mvp );
				renderer.Draw( va, ib, shader );
			}
